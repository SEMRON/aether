#!/usr/bin/env python3
"""
Single-shot evaluation script for wav2vec2 workers.
...
# Note: The subprocess "could not unlink shared memory file" error is non-fatal;
# the key fix is using shorter timeouts and avoiding the infinite retry loop.
...
from distqat.data import SpeechDataset, collate_fn
...
# Configuration
CONFIG_PATH = Path(__file__).resolve().parents[3] / "configs" / "wav2vec2.yaml"
INITIAL_PEERS_PATH = Path(__file__).resolve().parents[3] / "logs" / "wav2vec2_full" / "initial_peers.txt"
EVAL_RESULTS_DIR = Path(__file__).resolve().parents[3] / "logs" / "wav2vec2_full" / "eval_results"
MAX_BATCHES = 50
SPLIT = "validation"
...
class EvalParamMirror:
    ...
    # Shorter timeout for evaluation (2 minutes instead of 10)
    # We don't need as long since we can just retry next cycle
    ...
    def get_model(self) -> Optional[torch.nn.Module]:
        ...
        For wav2vec2 there's typically only one stage.
        """
...
def _log_to_wandb(metrics: Dict[str, Any], wandb_enabled: bool, index: int) -> None:
    ...
    try:
        ...
        logger.info(f"Logged metrics to wandb: eval/loss_{index}={metrics.get('loss', 'N/A'):.4f}")
    except Exception as e:
...
def _build_eval_dataset(cfg: Config, split: str):
    """Build speech dataset for evaluation."""
...
def _compute_ctc_loss(cfg: Config, inputs, outputs, labels) -> torch.Tensor:
    """Compute CTC loss for speech recognition."""
...
def run_evaluation(cfg: Config, model: torch.nn.Module) -> Dict[str, Any]:
    ...
    # Use num_workers=0 to avoid shared memory issues in subprocess cleanup
    loader = DataLoader(
        ds,
        batch_size=bs,
        num_workers=0,
    ...
    with torch.inference_mode():
        for i, (_uids, batch) in enumerate(loader):
            ...
            with autocast_ctx:
                ...
                loss_t = _compute_ctc_loss(cfg, inputs, outputs, labels)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Evaluate wav2vec2 model with parameter mirroring")
    parser.add_argument(
        "--index",
        type=int,
        default=0,
...
def main():
    ...
    try:
        ...
        model = param_mirror.get_model()
        if model is None:
            logger.error("No model initialized from parameter mirror")
            return 1
        ...
        metrics = run_evaluation(cfg, model)
        ...
        if "error" in metrics:
            ...
            logger.info(f"Evaluation completed. Loss: {metrics['loss']:.4f}")
