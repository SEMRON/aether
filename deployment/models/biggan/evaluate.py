...
# Configuration
CONFIG_PATH = Path(__file__).resolve().parents[3] / "configs" / "biggan_imnet.yaml"
INITIAL_PEERS_PATH = Path(__file__).resolve().parents[3] / "logs" / "biggan_imnet" / "initial_peers.txt"
EVAL_RESULTS_DIR = Path(__file__).resolve().parents[3] / "logs" / "biggan_imnet" / "eval_results"
CHECKPOINT_DIR = Path(__file__).resolve().parents[3] / "checkpoints" / "biggan_eval"
MAX_BATCHES = 20
EVAL_INTERVAL = 60
...
class EvalParamMirror:
    ...
    def __init__(self, cfg: Config, dht: DHT):
        ...
        for stage_index, pipeline_step_cfg in enumerate(cfg.model_pipeline.pipeline):
            ...
            loaded = self._load_state_with_retries(optimizer, stage_index)
            if loaded:
    ...
    def _load_state_with_retries(self, optimizer: object, stage_index: int) -> bool:
        """
...
def main():
    ...
    try:
        ...
        while True:
            ...
            metrics = run_evaluation(cfg, models, eval_step=eval_count)
