...
from distqat.data import SequencePackingDataset, SpeechDataset, CVDataset, collate_fn
...
# Configuration
CONFIG_PATH = Path(__file__).resolve().parents[3] / "configs" / "gptneo_split_2.yaml"
INITIAL_PEERS_PATH = Path(__file__).resolve().parents[3] / "logs" / "gptneo" / "initial_peers.txt"
EVAL_RESULTS_DIR = Path(__file__).resolve().parents[3] / "logs" / "gptneo" / "eval_results"
CHECKPOINT_DIR = Path(__file__).resolve().parents[3] / "checkpoints" / "gptneo_eval"
MAX_BATCHES = 20
SPLIT = "validation"
EVAL_INTERVAL = 300  # seconds between evaluations
SAVE_CHECKPOINTS = True  # Whether to save checkpoints after each evaluation
KEEP_HISTORY = False  # Whether to keep history of checkpoints
...
class EvalParamMirror:
    ...
    def __init__(self, cfg: Config, dht: DHT):
        ...
        for stage_index, pipeline_step_cfg in enumerate(cfg.model_pipeline.pipeline):
            ...
            # Initial parameter sync from peers
            try:
                self._load_state_with_timeout(optimizer)
                # Clone parameters to avoid shared memory issues
...
def _build_eval_dataset(cfg: Config, split: str):
    """Build dataset for evaluation based on task type."""
    ...
    if task_type == "cv":
        ...
        ds = load_dataset(
            data_cfg.dataset_name,
            split=use_split,
            streaming=True if data_cfg.dataset_path is None else False,
            token=data_cfg.hf_token,
            cache_dir=data_cfg.dataset_path,
        )
    ...
    raise ValueError(f"Unsupported task_type for evaluation: {task_type}")


def _compute_loss(cfg: Config, inputs, outputs, labels) -> torch.Tensor:
    """Compute task-appropriate loss."""
    task_type = cfg.data.task_type

    if task_type == "llm":
        # Language model loss: cross-entropy on shifted logits
        return outputs.mean()
    ...
    if task_type == "speech":
        # For speech, just return mean of outputs as placeholder
        # Real CTC loss requires more complex handling
        return outputs.mean()
    ...
    with torch.inference_mode():
        for i, (_uids, batch) in enumerate(loader):
            ...
            with autocast_ctx:
                ...
                loss_t = _compute_loss(cfg, inputs, outputs, labels)
    ...
    try:
        ...
        while True:
            ...
            if "error" in metrics:
                ...
                _log_to_wandb(metrics, wandb_enabled, eval_count)
