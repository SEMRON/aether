...
def head_sample_input(batch_size, in_dim, action_dim):
    return (torch.empty((batch_size, in_dim)), torch.empty((batch_size, action_dim)))
...
class PPOAgent(nn.Module):
    ...
    def forward(self, x, action):
        # workaround to satisfy remote experts forward schema
        if action.mean() == 0:
            action = None
...
def compute_gae_returns(
    ...
    with torch.no_grad():
        ...
        no_action = torch.zeros((next_obs.shape[0], action_dim))
        _, _, _, next_value = agent((next_obs, no_action))
        next_value = next_value.reshape(1, -1).to(device)
