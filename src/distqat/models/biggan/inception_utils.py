...
# Streaming-compatible: compute real-data moments from an iterator/loader
def accumulate_inception_activations_from_loader(loader, net, num_images=50000):
  pool = []
  seen = 0
  for batch in loader:
    if isinstance(batch, (list, tuple)):
      ...
      images = batch
    with torch.no_grad():
      pool_val, _ = net(images.float().cuda(non_blocking=True))
    pool.append(pool_val)
    seen += pool_val.shape[0]
    if seen >= num_images:
      break 
  pool = torch.cat(pool, 0)[:num_images]
  return pool


def prepare_inception_metrics_from_loader(real_loader, parallel, num_real_images=50000, no_fid=False):
  net = load_inception_net(parallel)
  # Compute real dataset moments on the fly
  with torch.no_grad():
    real_pool = accumulate_inception_activations_from_loader(real_loader, net, num_images=num_real_images)
    real_mu = torch.mean(real_pool, 0)
    real_sigma = torch_cov(real_pool, rowvar=False)
  del real_pool
  ...
  def get_inception_metrics(sample, num_inception_images, num_splits=10, prints=True, use_torch=True):
    ...
    if no_fid:
      ...
      if use_torch:
        ...
        if prints:
          print('Covariances calculated, getting FID...')
        FID = torch_calculate_frechet_distance(mu, sigma, real_mu, real_sigma)
        FID = float(FID.cpu().numpy())
      else:
        mu_np = np.mean(pool.cpu().numpy(), axis=0)
        sigma_np = np.cov(pool.cpu().numpy(), rowvar=False)
        FID = numpy_calculate_frechet_distance(mu_np, sigma_np, real_mu.cpu().numpy(), real_sigma.cpu().numpy())
    del mu, sigma, pool, logits, labels
    return IS_mean, IS_std, FID
