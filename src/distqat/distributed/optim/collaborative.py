...
class CollaborativeOptimizer(DecentralizedOptimizerBase):
    ...
    :param params: an iterable of parameters to optimize or dicts defining parameter groups. Only used when optimizer is a callable.
    :param dht: a running hivemind.DHT daemon connected to other peers
    :param run_id: a common prefix for all metadata stored by CollaborativeOptimizer in the DHT
    ...
    :param verbose: if True, log collaboration state updates at INFO level, otherwise at DEBUG level
    ...
    def step(self, batch_size: Optional[int] = None, **kwargs):
        ...
        logger.log(self.status_loglevel, f"{self.run_id} beginning global step #{self.collaboration_state.optimizer_step}")
        self.collaboration_state = self._fetch_state()
        ...
        with self.performance_ema.pause(), self.lock_collaboration_state:
            ...
            if self.collaboration_state.num_peers > 1:
                ...
                logger.log(
                    ...
                    f"Skipped averaging: {self.run_id} consists of " f"{self.collaboration_state.num_peers} peer(s).",
                )
    ...
    def step_aux(self, **kwargs):
        ...
        :param kwargs: additional parameters forwarded to TrainingAverager.step
        :returns: group_info dict containing information about the averaging group if averaging was performed, None otherwise
        :note: this .step is different from normal pytorch optimizers in several key ways. See __init__ for details.
        """
        ...
        logger.log(self.status_loglevel, f"{self.run_id} beginning global optimizer step #{self.collaboration_state.optimizer_step}")
        self.collaboration_state = self._fetch_state()
        self.collaboration_state_updated.set()
    ...
    def _fetch_state(self) -> CollaborationState:
        ...
        logger.log(
            self.status_loglevel,
            f"{self.run_id} accumulated {total_samples_accumulated} samples from "
