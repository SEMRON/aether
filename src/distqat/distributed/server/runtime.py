...
class Runtime(threading.Thread):
    ...
    def run(self):
        """
        Main runtime loop that processes batches from expert pools.
        
        Starts all expert pools, then continuously processes incoming batches
        from experts, handles them through the appropriate forward/backward pools,
...
class StatsReporter(threading.Thread):
    """
    A background thread that collects and reports runtime performance statistics.
    
    This thread periodically aggregates batch processing statistics from expert pools
    and logs performance metrics like throughput and processing times.
    
    :param report_interval: how often (in seconds) to collect and report statistics
    """
    ...
    def run(self):
        """
        Main loop that collects and reports runtime statistics.
        
        Periodically aggregates batch processing statistics from all expert pools
        and logs performance metrics like throughput and processing times.
        """
    ...
    def report_stats(self, pool_uid, batch_size, processing_time):
        """
        Report batch processing statistics for aggregation and logging.
        
        :param pool_uid: identifier of the expert pool that processed the batch
        :param batch_size: number of examples in the processed batch
        :param processing_time: time taken to process the batch (in seconds)
        """
