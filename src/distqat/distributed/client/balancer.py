...
from hivemind.utils import TimedStorage, DHTExpiration, ValueWithExpiration, get_dht_time, get_logger, PerformanceEMA

from distqat.distributed.client.expert import RemoteExpert
from distqat.distributed.server.expert_uid import ExpertPrefix, ExpertUID
...
class ExpertBalancer:
    """
    Manages a pool of remote experts and provides load balancing for expert selection.
    
    This class maintains a dynamic pool of experts discovered through DHT, tracks their throughput
    performance, and provides load-balanced access to experts based on their current load and
    historical performance metrics.
    
    :param dht: Distributed hash table for expert discovery and coordination
    :param key: Expert prefix to search for in the DHT
    :param update_period: How often to refresh the expert pool from DHT (in seconds)
    :param initial_throughput: Initial throughput estimate for new experts (tasks per second)
    :param sleep_timeout: Time to sleep when no experts are available (in seconds)
    :param kwargs: Additional keyword arguments passed to PerformanceEMA for throughput tracking
    """
    ...
    def use_another_expert(self, task_size: float) -> RemoteExpert:
        """
        Context manager that provides access to a load-balanced expert for task execution.
        
        Selects the expert with the lowest predicted completion time based on current load
        and historical throughput. Updates the expert's load estimation and performance metrics.
        
        :param task_size: Size of the task to be executed (used for load balancing)
        :yields: RemoteExpert instance ready for task execution
        :raises: Various exceptions if expert selection or communication fails
        """
        while True:
            if len(self.queue) == 0:
    ...
    def shutdown(self):
        """
        Gracefully shutdown the expert balancer and stop background update thread.
        
        Signals the update thread to stop and waits for it to finish processing.
        """
        self.is_alive.clear()
        self.update_finished.clear()
        self.update_trigger.set()
